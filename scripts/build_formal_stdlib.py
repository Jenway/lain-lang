...
The result is deliberately separate from ``build/bootstrap/stdlib.l1``.  The latter
...
ABI_SUPPORT_ENTRIES = (
    ...
    "meta_syntax_context_source",
    "meta_syntax_context_append",
    "meta_ast_fresh_atom",
    "meta_syntax_context_release",
    "meta_ast_new_atom",
    "meta_ast_new_group",
    "meta_ast_append",
    "meta_ast_replace",
    ...
    "meta_binding_returns_name",
    "meta_operator_code",
    "meta_literal_length",
    "meta_copy_literal",
    "meta_copy_source",
    "meta_copy_struct_field",
    ...
    "meta_copy_call_two_local_arguments",
    ...
    "meta_syntax_index_source",
    "meta_syntax_index_root",
    "meta_syntax_index_imports",
    "meta_import_status",
    "meta_is_import",
    "meta_import_is_std",
    "meta_import_target",
...
def verify_abi_entry() -> None:
    ...
    selected = subprocess.run(
        [
            ...
            str(ABI_OUTPUT),
            str(ROOT / "bootstrap" / "compiler" / "compiler_api.l1"),
        ],
        cwd=ROOT,
        capture_output=True,
        text=True,
    ...
    macro_multi_decl_probe = subprocess.run(
        [
            str(SEED_RUN),
            "interpreter",
            str(ABI_PROBE),
            ...
            str(ABI_OUTPUT),
            str(ROOT / "bootstrap" / "compiler" / "compiler_api.l1"),
            str(FORMAL_MACRO_TWO_DECLARATIONS_PROBE),
        ],
        cwd=ROOT,
        capture_output=True,
        text=True,
    )
    if macro_multi_decl_probe.returncode:
        detail = (
            macro_multi_decl_probe.stderr.strip()
            or macro_multi_decl_probe.stdout.strip()
        )
        raise RuntimeError(
            "formal stdlib multiple macro declarations failed"
            + (f": {detail}" if detail else "")
        )
    macro_multi_decl_run = subprocess.run(
        [str(SEED_RUN), "run", str(ABI_OUTPUT), "main"],
        cwd=ROOT,
        capture_output=True,
        text=True,
    ...
    if (
        macro_multi_decl_run.returncode
        or macro_multi_decl_run.stdout.strip() != "21"
    ):
        detail = (
            macro_multi_decl_run.stderr.strip()
            or macro_multi_decl_run.stdout.strip()
        )
        raise RuntimeError(
...
def verify_meta_diagnostics() -> None:
    ...
    if META_DIAGNOSTIC_OUTPUT.read_text(encoding="utf-8") != "status=1 token=1 origin=1 context=1 fresh=tmp ctx=3\n":
        raise RuntimeError(
            "formal Meta diagnostic probe did not return lexical status, token match, origin, context and fresh atom"
        )
