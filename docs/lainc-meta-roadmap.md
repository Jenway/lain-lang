...
> `lower` 已能为十进制常量 `return` 生成并运行最小 LAIN-IR，其他形式明确
> 返回迁移中的 `5203`。
...
- [~] Meta 工厂内部的局部 `std::func`、`Module`/`ModuleShape` 返回值和
  ...
  `5203`；十进制常量 `return` 已完成正式库 lowering 和 artifact 回灌，复杂
  表达式、调用、控制流和完整 L1 Unit 仍待迁移。
- [~] 每个正式库导出与 Bootstrap Standard Library ABI v1 对应的入口；
  ...
  `return` 已生成可执行 artifact，其余形式明确返回 `5203`。
