...
- **第一代已归档**：`archive/**` 承载第一代 C 运行时与手写 LAINIR 前端；`std/` 里绑在第一代宿主
  ABI 上的两份（`meta.lain`、`bootstrap/abi_entry.lain`）也已移入 `archive/std/`。
  **第一代不再承担新主线的前置条件。**
- **未来主线围绕四项可执行证明**：Meta 可替换 / 编译期结果可被 Meta 消费 / 类型与模块的公开契约
  成立 / 正式编译器可运行并固定点。见 §5。
- **编码 1–7 的边界原则继续有效**（§4、§7.0），但执行位置从第一代链移到了第二代；逐条归属见 §5.3。
...
| 位置 | 职责与修改约束 |
| --- | --- |
| `seed/src/**` | 第二代的 C 物理 parser/verifier/VM/C 后端/fold 与 Meta 宿主（**11 个通用能力**）；仅补真实缺失的物理机制，不实现 Lain 语言规则 |
| `seed/bootstrap/**`、`seed/lain/**` | **第二代 Meta**（手写 LAINIR，3 个入口：`lain_std_abi_version/initialize/lower`）与它的 root 环境 `seed/lain/std/prelude.lain`。**新主线在此推进** |
| `archive/**` | **第一代已归档**：C 运行时（14003 行）、手写 LAINIR 前端（22190 行）、以及 2026-09-20 从 `std/` 移入的 `archive/std/meta.lain`（178 个 `@abi_export`、61 个 `@foreign`）与 `archive/std/bootstrap/abi_entry.lain`。只读参考，不进构建闭包，**不承担前置条件** |
| `src/lainc/` | 正式 Lain 编译器——**目标，不是前置**。其 `src/lainc/meta.lain:15`、`src/lainc/types.lain:13` 的 `import("std::meta")` 现指向 `archive/std/meta.lain`，需按第二代 ABI 重写，或按同一判据归置 |
| `std/`（除已归档两份） | 正式库的数据结构与策略（`core/*`、`platform/*`、`effect.lain`、`bounds.lain`、`type_policy.lain` …），**不含任何 `@foreign`**。是待实现的**目标**，不是第一代接口 |
| `src/lainir/`、`src/lainvm/` | 活跃契约；正式 provider/VM 的运行能力提供方式待编码 5 决定 |
| `build/` | 所有生成产物、快照、差分报告；永不提交，不修改产物来通过验收 |

> **判据（2026-09-20）**："与第二代不符合"的机械判据是**有没有 `@foreign` 绑在第一代宿主能力名上**
> （`lain_ast_v1_*`、`tool_*`、短横线命名 `bootstrap.*`）。按此判据只有上面那两份需要归档。
> 注意区分两种"不符合"：**接口契约层面不符合**（拆）与 **gen-2 今天解析不了**（是目标，不是拆的理由
> —— 例如 `std/core/vec.lain` 只是 v0 输入语言还读不了它）。
...
**2026-09-20 新增**（依据评审 §1.3、§3、§6、§7）：

- **两套 Meta ABI 并存且从未调和。** `archive/std/meta.lain`（178 个 `@abi_export`、61 个 `@foreign`）
  与第二代宿主的 11 个能力**交集为 0**。第二代不是"补齐"了它，而是换了一套 ABI。
- **"Meta 可替换"没有验收定义。** 三个技术前提都不具备：暂存区布局是隐式契约、没有规范文本语料、
  Meta 清单在驱动里写死（`seed/tests/meta_boot.c:302`）。
- **`#eval` 通路断开。** C 侧有折叠器（`seed/src/fold/fold.c`，390 行）且 C 后端要求先折叠
  （`seed/src/backend/cbackend.c:1227`，否则 9225），但宿主 11 个能力里没有求值，
  `seed/tests/meta_boot.c` 也不调用 fold。
- **表达式没有自己的类型信息**，因此 `if`、嵌套表达式、字段访问共同受阻（`for` 能用 `<` 是因为
  它的语法写死了"这里要一个比较"）。
- **类型与模块距 ML 式机制三块前置**：类型身份 key 无实参（`arg_*` 恒空）、`struct` 不是一等记录
  （布局每次回源码重读）、模块是"源码下标 + mangle"。
- **自举距离未测量**：v0 输入语言能否表达 4938 行的 Meta，没有证据。
...
未来主线围绕这四项推进。每一项目前都**不可判定**；把它们变成可判定就是近期工作的定义。

| # | 证明 | 今天不可判定的原因 |
| --- | --- | --- |
| P1 | **Meta 可替换** | 无验收定义；暂存区布局是隐式契约；无规范文本语料；Meta 清单在驱动里写死 |
| P2 | **编译期结果可被 Meta 消费** | `#eval` 通路断开（§3）；且只在最后折叠时，早期类型检查消费不到结果 |
| P3 | **类型与模块的公开契约成立** | 参数区恒空；`struct` 不是一等记录；模块是源码下标 + mangle |
| P4 | **正式编译器可运行并固定点** | 依赖 P2/P3；固定点定义为代际实际执行 + 规范化比较 + 独立预期 |

### 5.2 里程碑（活动顺序）

命令均为**拟新增或拟扩展**，不表示今天存在。退出码 `0=pass / 1=fail / 2=missing`；
缺前置不得记 SKIP 通过。每步都必须跑 `--suite mainline` 作为回归。

| 里程碑 | 前置 | 完成条件 |
| --- | --- | --- |
| **M0 基线** | 已测 84 项 | `python scripts/check_seed_meta.py --suite mainline`：从第二代当前源重建、84 项保持全过、产物来源与编译命令存入 `build/`，输出 `PASS mainline` |
| **M1 替换 + 表达式基础** | M0 | `--suite replaceability`、`--suite expression`：规则替换、内部表示替换、**独立极简第二 Meta**、A/B/A 无残留；表达式类型正反例全过 |
| **M2 求值闭环 + 首个 effect** | M1；§15 的 E1/E2 已决 | `--suite eval`、`--suite effect`：Meta 消费真实结果继续生成程序；handler 改变输出；负例与资源失败通过 |
| **M3 类型/模块核心** | M1、M2；E3 已决 | `--suite module`：签名隐藏、共享等式、名义区分、工厂参数与重复应用身份按契约运行 |
| **M4 正式编译器可执行 + 固定点** | M2/M3；正式源码所需能力全部有证据；E5 已决 | `python scripts/check_lain_selfhost_execution.py`：G1/G2/G3 真正执行，G2/G3 规范化相同，共同行为集全过 |
| **M5 稳定交付** | M4；性能与物理差分基线 | `python scripts/check_lain_release_contract.py`：干净重建、选定平台矩阵、Meta 更换、固定点与 ABI 负例通过 |

**已完成（2026-09-20）——P1 已经可判定**：

| 切片 | 验收命令 | 结果 |
| --- | --- | --- |
| M0 基线 + 产物来源 | `python scripts/check_seed_meta.py --suite mainline` | `ALL PASS (3/3)`；来源（源码摘要 + 编译命令 + 驱动摘要）记在 `build/seed-meta/mainline-provenance.json` |
| 规范化文本语料 | `python scripts/check_seed_meta.py --suite canonical` | `ALL PASS (14/14)`，含"同输入两次整份输出逐字节一致" |
| 只换 Meta、不重编驱动 | `python scripts/check_seed_meta.py --suite replaceability` | `ALL PASS (24/24)` |

替换验收的具体形状：`seed/tests/meta_conformance/meta_min.l1` 是一份**独立**的极简第二 Meta
（8686 字节；与主线 Meta **不共享任何代码**，连空白/注释跳过、标识符判断、关键字匹配、
`#extern` 声明都自己写；只调 5 个宿主能力，**完全不碰暂存区**）。它在 core 档 6 条上与主线 Meta
**逐字节相同**，在 full 档 8 条上**全部响亮拒绝**（status 4）——后者证明语料对两份 Meta 有区分力，
不是空转。同一份宿主二进制（sha256 未变）跑完 A/B/A 后行为不变。证据：第二 Meta 解析
**15** 个能力引用，主线同输入是 **173**。

**尚未做**：M1 的"表达式类型基础"要改语言核心（`funcs.l1`/`scope.l1`/`registry.l1`），
本轮按约束未动；另有 4 项待决定见 `docs/implementation/pending-decisions.md`（D9–D12）。

> 入口数量上限：**最多新增 2 个脚本**，用 `--suite` 区分。仓库已有 53 个 `check_*.py` 与 36 道 gate
> 的编排入口；再加 9 个会让"哪条是当前主线"重新变模糊。

### 5.3 编码 1–7 的逐条归属

原表的工作**不删除**，但执行位置从第一代链移到第二代；第一代不再作为前置。

| 阶段 | 处置 | 说明 |
| --- | --- | --- |
| 编码 1 | **保留为边界原则，换执行位置** | §7.0 的归属原则（core 只组合通用能力、禁止语言专用宿主原语、禁止绕过 `#eval`）继续有效并适用于第二代；§7.1 的迁移清单按该节的 2026-09-20 注记更新 |
| 编码 2（剩余） | 保留，并入 P3 | 正式签名语义、类型检查与未覆盖形状一致性 |
| 编码 3（剩余） | 保留，并入 P3 | 完整库策略输入与类型约束 |
| 编码 5 | 保留，并入 P2 | 编译期求值组合；provider 路径选择见 §15 E5 |
| 编码 6 | 保留，即 P4 | 固定点；定义见 §5.2 的 M4 |
| 编码 7 | 保留，可独立推进 | 后端语义覆盖、native matrix、发布验收 |

编码 2/3 的剩余库语义工作与编码 1 一并安排，避免重复建设。每片具有独立执行验收，
完成后从本表及正文移除对应工作。
...
本阶段的**边界原则**继续有效，但执行位置改为第二代（§5.3）。

> **2026-09-20 注记**：原文"本阶段先在 bootstrap 链证明上述边界"已不适用——第一代链无运行闭环
> （实测 `run_lain_compiler.py` 10 分钟零输出后超时；`bootstrap/README.md` 自述该源码集"故意不完整"）。
> 原文"`bootstrap/std/` 与 `std/` 共用 Meta ABI 契约"也已不成立：`std/meta.lain` 与
> `std/bootstrap/abi_entry.lain` 已移入 `archive/std/`，而第二代宿主只有 11 个能力，与该契约
> **交集为 0**（评审 §1.3）。

Lain 语言语义不得通过修改 C seed 实现。正式 lainc 的运行组合仍属编码 5。
...
archive/std/meta.lain
archive/std/bootstrap/abi_entry.lain
```

> 后两条已于 2026-09-20 移入 `archive/std/`（提交 `4d876d9`）。它们绑在第一代宿主能力名上
> （30 个 `lain_ast_v1_*` + 25 个 `tool_*` + `raw_parse` + 5 个短横线 `bootstrap.*`），
> 与第二代宿主的 11 个能力交集为 0，因此**不是待迁移的清单，是第一代的 ABI 记录**。
> 其余 `bootstrap/**` 条目同属第一代，按 §5.3 保留为边界参考、不作为前置。
...
**2026-09-20 新增**（依据评审 §11，均涉及跨边界 ABI 或身份语义，代价高）：

- **E1 Meta 怎样请求物理执行并继续。** 当前 11 个能力与驱动入口是否足以承载求值往返，无执行证据。
  两个方案：保持能力集合、明确分阶段请求与续行协议（须证明现有输出通道允许这样用，不能暗中改成
  RPC）；或批准一个最小的通用物理执行 ABI（禁止 type/module/effect 分支）。
- **E2 Meta 对象引用与持久化结果。** 某个 Meta 值无法安全用现有 bits/addr/unit 传递，或引用需跨越
  当前执行生命周期时停下。两案：对象保留在明确授权、生命周期足够长的编译期内存区；或跨阶段只交付
  可重建的物理数据。**不得把临时 VM 地址写进 native 产物。**
- **E3 模块工厂的类型身份。** 重复应用同一工厂与同一模块参数，是保留类型兼容性还是显式产生新身份。
  **不能让"缓存命中返回旧 ID、未命中生成新 ID"偶然替代它。**
- **E4 首个 handler 的恢复范围与资源规则。** 非尾恢复、恢复次数、捕获状态与资源退出的可执行覆盖
  未提供。若既定设计已规定恢复语义，不能为了易实现擅自更改。
- **E5 正式编译器依赖的物理运行能力。** 适配现有 C 物理能力，或延后组合、先让一个 Lain 物理 provider
  通过独立执行契约。**不能把归档实现"可编译"当作完成。**
