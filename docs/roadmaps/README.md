...
当前路线图只有 [`lain-roadmap.md`](lain-roadmap.md) 一份。错误的求值结果协议已经删除，`seed`、`bootstrap` 和 `src` 已经分离；C seed 已通过临时 TCB 执行 `#eval`。下一阶段让 Lain 实现相同的 LAINVM 语义，并把执行代码迁入 `src/lainvm/`。
