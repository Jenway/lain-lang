...
##### 第八个缺陷：`#sdiv` 对负数结果错误（2026-09-12，已修）

再扩面到「负数除法」后，差分测试报出：

```
differential_signed_division.l1: seed returned '3', Lain backend returned '2147483647'
```

fixture 计算 `sdiv(-7,2) + sdiv(7,2) + sdiv(-7,-2)`，正确值是 `-3 + 3 + 3 = 3`。

**根因**：`src/lainc/backend_c.lain` 的 `L1_sdiv(a,b)` 定义为 `((int64_t)(a)/(int64_t)(b))`，
而本后端把 `#bits<32>` 映射为 **`uint32_t`**。把 `uint32_t` 升宽到 `int64_t` 是**零扩展**：
`-7` 在 `uint32_t` 里是 `4294967289`，`/2` 得 `2147483644`。

seed 无此问题：它把 `#bits<32>` 映射为 `int32_t`，`a / b` 直接就是有符号除法。

**关键**：错在宏**升宽**这一步，而不是除法本身。正确做法是按**操作数自身宽度**重新解释为有符号
（对 `#bits<32>` 即 `(int32_t)`），而不是升到一个更宽的类型。

**这条与之前刻意保留的 `#bits<32>` 无符号决定直接相关**：§13.0.1 记过「把 `#bits<32>` 从
`uint32_t` 改成 seed 的 `int32_t` 会扩散 125 处、含 14 处 host ABI 原型」，当时选择保留。
现在这个选择**溢出了**：它让依赖有符号解释的操作（`#sdiv`）需要额外的按宽度重解释。
这不是推翻那个决定，而是它的已知代价第一次显形——**记录在案**。

**当前七个 fixture 全部一致**：`100`、`32`、`-41`、`10110`、`4508`、`31`，加上本条修好后的一致值。

##### 第七次扩面：`#alloca` 的两种实现不一致（2026-09-12，已记录未修）

扩面到 `#lea` 与 `#alloca` 时，差分测试报出 `seed 31 / Lain 29`（差 2）。定位后发现**不是地址计算错**，
而是 **`#alloca` 的两处实现语义不同**：

| | seed | Lain 后端 |
| --- | --- | --- |
| 分配 | `((uint8_t[N]){0})` —— **复合字面量，零初始化**，随 activation 结束而失效 | `malloc` —— **未初始化**，且**从不 free** |

差 2 来自 fixture 里「读一段未写过的 alloca 内存」，seed 得 0、Lain 得垃圾值。
**fixture 的问题已修**（改为先写后读，不再依赖未初始化内存），但两处实现的分歧是真实的：

1. **零初始化**：seed 清零，Lain 不清零；`docs/01-lain-ir.md` **未规定**。因此读未写过的 alloca
   内存在 Lain 后端下**不确定**，与规范 §9 对确定性的要求相悖（该要求针对编译器产物，
   但同一性质也影响运行期）。
2. **生命周期**：规范写明「返回时释放其中全部 `#alloca`」，而 `malloc` 版**泄漏**。
   这在单次调用里不可观测，量大时不是。
3. 规范另写「地址不得从这个 activation 逃逸。参考解释器会拒绝直接返回本次 activation 的
   `#alloca` 地址」——**两个 C 后端都无法强制执行**该检查（seed 靠 C 作用域偶然接近，
   Lain 完全没有）。

**处置建议**：属编码 7「C backend 剩余物理指令和 ABI」的范围——需要先定「`#alloca` 是否必须清零」
与「activation 结束时如何释放」，再改后端。**当前只记录，不改**，因为清零是 O(n) 成本且规范沉默，
不该由我单方面决定。
...
| `#bitcast` | **0** | **0** | 0 |
| `#proc_addr` | **0** | **0** | 0 |
| `#trunc`/`#sext`/`#zext` | 支持 | 曾匹配但**不发射**（第六个缺陷，已修） | — |
