...
| 编码 3 | **部分完成 2026-09-12**：调用点解析与三类诊断已落地；推导与 scalar 路径未完成（§9.0.1） | `?{}` 能推导并从环境解析输入 |
...
#### 尚未完成的部分（必须一并阅读，不得据此认为编码 3 已完结）

1. **值输入的类型匹配只做到 kind 级**：Meta scalar **值**不携带宽度（`meta_value_scalar` 的
   payload 是 `{number, 0}`），所以 `?{x: i64}` 会接受一个 `i32` **值**。注意这与下面第 2 项
   不同：**类型值**（如 `?{T: std::type}`）现在能给出宽度，因此依赖形参宽度已经可用。
2. **§9.2 的约束来源实现了 2/4**（2026-09-12）：来自**显式参数类型标注**与**返回类型标注**的
   约束已实现——裸名 `?{T}` 若出现在类型位置，其 kind 推定为类型值（不再 5104），否则仍 5104。
   仍未实现：**同行其它条目的类型表达式**、**body 内的约束**。因此 `?{T, ord: Ord(T)}` 里的 T
   不会由 `Ord(T)` 推导。
...
后续片应从**第 2 项的剩余两个约束来源**入手（同行条目类型表达式、body），或先补齐第 1 项
所需的 Meta 值宽度表示。§9.4 原文正例还额外缺一个**库符号 `Ord`**（全仓库不存在），创建它
属库的工作。
