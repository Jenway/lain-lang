...
| 阶段 | 状态 | 已有证据 | 尚缺内容 |
| --- | --- | --- | --- |
| 0 行为基线 | 部分完成 | seed recording provider 覆盖明确整数、memory、data 与 eval；legacy 语法拒绝检查已建立 | compiler 成功/失败 fixture 与 canonical 快照 |
...
| 5 Meta eval | 已完成调用面迁移 | Meta 只调用 Eval API，不读取 IR 表 | owner、limit、capability 与嵌套 eval 语义 |
