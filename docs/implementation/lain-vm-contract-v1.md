...
root instruction 继续；两层 `wrapper -> leaf` 的 Endpoint A/B 顺序等待 fixture 固定了这一点。
...
handle 引用了 VSpace backing storage，还必须携带创建时的 VSpace identity 和 generation；
另一个同 owner、同 generation 的 VSpace 也不能接受该 handle。VSpace reset 或 release
后，即使 owner 没变，也必须拒绝再次使用该 handle。
