...
8. **当前阶段：provider payload adapter 与 lowering 前置。** 为 addr、string、func 及
   ...
   规则。seed 已提供 `LainirVmResultAdapter` 和
   `lainir_vm_result_handle_adapt`：adapter 必须携带 object kind、owner、session
   generation 和 borrowed/owned 标记；owned 必须注册 destructor，borrowed 不得注册释放
   ...
   验收接入 native/formal matrix。formal provider 的 VSpace 已把 generation 写入每个
   `Address`，地址解析和 `#lea` 派生都会拒绝过期 generation；release 会递增 VSpace
   generation。LAINIR 的 `EvalShape` 已把 object 判断、kind、owner、
...
析构边界、adapter ownership 规则和无 adapter 的 backend 拒绝门槛已有 fixture，
provider-specific adapter 仍是当前阶段。
