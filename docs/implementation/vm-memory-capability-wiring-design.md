...
## 4 裁定三：C 后端第一期不跟随（精确记缺口，不自行改多后端 ABI）

`seed/src/backend/cbackend.c` 的 `#addr` 就是 `uintptr_t`（`width_of_type:316`、load/store `:789-816`
按 8 B、`INST_ALLOCA:817` 指静态 C 数组）。第一期**不改它**，边界精确记成：

- **同帧内用法两边一致**：`#alloca` 在 C 后端是一块真实的静态存储，同帧内 store/load 的读写语义
  与解释器一致（`seed/tests/emit_c.c` 与 `cbackend_driver.c` 的 `alloca_store` 用例本来就是这块覆盖）。
- **缺口只有一处**：**逃逸**（过程返回后拿旧引用访问）在解释器被拒（实测 9207），C 后端没有对应
  检查——那块存储还在，于是能读到。也就是说 **C 后端是"未受检后端"**。
- 改它等于同时改多后端 ABI（`#addr` 在 C 里要不要也变成两个字的记录、load/store 宽度跟不跟），
  属于交接 §1 点名要作者确认的一项，所以**不自行决定**：这一片不做实现，只把缺口记在这里。
  拒绝整块 `#alloca`/`#int2ptr` 不是选项——那会把 `alloca_store` 这类**正确**的同帧用法一起拒掉。
