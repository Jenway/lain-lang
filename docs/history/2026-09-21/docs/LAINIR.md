> 历史记录。原路径：`docs/LAINIR.md`。归档日期：2026-09-21。
> 本文保留整理前的内容；其中的状态、命令、语法和结论不作为现行依据。
> 当前文档从 [文档索引](../../../README.md) 阅读。

# LAINIR

**状态**：规范 v0；`seed/` 里有一个实现，覆盖到什么程度见下。
**范围**：LAINIR 的物理形态。不含语言语义，不含内存管理，不含 ABI。

已实现并验收：文本两端（parse / print，canonical 固定点）、验证器、解释执行、
C 后端、`#eval` 折叠、域外行为的确定性拒绝、能力的宿主 ABI。
**还没实现**：向量算子、原子算子（`seed/src/vm/engine.c` 里是 stub），
以及后端和引擎里都没有的 `#eval` 检查消去。
...
- `#switch` 无 fallthrough，case 值必须是常量；**要带选择子的类型实参**
  （`#switch[#bits<32>] %k`）——引擎和后端都靠它把常量掩到选择子的宽度上，
  否则写宽了的常量在两边会有不同的匹配行为。default 语义必须显式。
- 终结子之后不得再有指令。
...
8. 搬运操作必须带类型。
9. 操作数必须是值，不得是嵌套操作。
10. `#if` 的条件必须是 `#bits<1>`；`#switch` 必须带选择子类型实参、case 值不得
    重复、default 必须显式（2029），且每个分支的结果数与 default 一致。
