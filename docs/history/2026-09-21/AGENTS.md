> 历史记录。原路径：`AGENTS.md`。归档日期：2026-09-21。
...
- **整数「字面量自己有类型」（参考经典 C）**：装得下 `i32` 就是 `i32`，否则 `i64`；`i8`/`u8`/`bool` **不进候选**（C 也不把字面量定成 char/short）。**顺序是先按上下文的类型查**，上下文那个类型查不到这个算子（`bool` 的表是空的）才退回字面量自己的类型；**只在两个操作数都是裸字面量时**才退（有名字操作数时退会把 `x: u32` 的 `<` 发成 `slt`）。退回之后指令的类型实参要跟着换成 i32/i64（实现见 `meta_op_resolve_default`，宽度写在暂存区 `+488`）。
- **字面量装不进声明的类型 → 拒（码 23）**。`let a: i8 = 300;` 曾经静默截断成 44、`let a: bool = 2;` 截断成 0。宽度按**无符号**算（标量的 repr 不带符号，`i32`/`u32` 都是 `bits<32>`）。
- **诊断码**：**0 = 成功，非 0 = 稳定诊断码**。Meta 侧见过/实测过：3 缺操作数、4 值后面有垃圾、6 类型的 op 表里没这个算子、9 投了声明里没有的变体、10 投影无载荷的变体、11 模块找不到、12 不是 import 绑定、14 标量只能写在模块里、16 同层重名、17 暂存区装不下、20 函数体没有 return、21 语句缺 `;` 或局部变量缺类型标注、22 重复绑定、23 字面量装不进声明的类型；验证器另有 1004 地址未授权、2001 同区域重复绑定、2002 引用未定义的值、2005 算子的操作数类型不对、2019 返回类型不符。
