...
两种形态，都由折叠（folding）阶段算掉；后端输入不能残留待执行的 eval。

1. **带标记的调用**：`#eval callee(args)`。它是调用，只是标注了编译期求值阶段；
   实参必须在编译期已知（否则验证器拒 **2012**）。文本与打印互为逆（见下面的例子）。
2. **编译期块**：`%v = #eval -> (T) { ... }`。块拥有自己的区域，`-> (T)` 声明块的类型；
   块里每条路径都必须 `#return`，返回值的类型必须与声明一致（拒 **2006** / **2019**）。
   块的结果类型不得含 `#addr`——块里可以随便用地址，算完不许把地址带出来（拒 **2030**）。
   验证器把块当作**返回该类型的匿名过程**；引擎与后端都不该见到它
   （引擎 **1045**、C 后端 **9225**，都是稳定拒绝，不是静默执行）。
...
上段就是当前可编译的文本：parser 接受 `#eval callee(args)`（与 `#call` 同一条路径，
只多一个编译期标记），打印器打回同一种拼写，往返逐字节相同。
`seed/tests/eval_checks.c` + `scripts/check_eval.py` 是这一节的验收入口
（`block_ok`、`eval_call_ok` 两条正例与 `block_no_return`/`block_bad_return`/
`block_addr_result`/`block_engine_rejects`/`block_backend_rejects` 五条反例）。
编译器把版本实现为 `INST_EVAL`（块）与 `INST_CALL` + `is_eval`（调用）两条通路。
