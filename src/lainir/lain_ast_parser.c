...
//
// 核心创新：并列 (Juxtaposition)
//   两个表达式在没有显式操作符的情况下相邻出现时，
//   被解析为以 " " (空格) 为隐式操作符的 INFIX 节点。
//   隐含并列的绑定力最低 (prec=1)，所有显式操作符都比它紧。
...
static void lexer_skip_whitespace_and_comments(Lexer *l) {
    while (l->pos < l->len) {
        ...
        if (isspace(c)) { lexer_advance(l); }
            ...
            while (l->pos < l->len) {
                if (lexer_peek(l) == '*' && l->pos + 1 < l->len && l->src[l->pos + 1] == '/') {
                    lexer_advance(l); lexer_advance(l); break;
                }
                lexer_advance(l);
            }
        }
...
static Token lexer_next(Lexer *l) {
    ...
    if (l->pos >= l->len) {
        t.type = TOK_EOF; t.length = 0; return t;
    }
    ...
    if (c == '"') {
        ...
        t.type = TOK_STRING; t.length = (uint32_t)(l->src + l->pos - t.start);
        return t;
    }
    ...
    if (c == '\'') {
        ...
        lexer_advance(l);
        if (l->pos < l->len) lexer_advance(l);
        t.type = TOK_CHAR; t.length = (uint32_t)(l->src + l->pos - t.start);
        return t;
    }

    if (isdigit(c)) {
        while (l->pos < l->len && (isdigit(lexer_peek(l)) || lexer_peek(l) == '.' || lexer_peek(l) == '_'))
            lexer_advance(l);
        t.type = (t.start[0] == '0' && (t.start[1] == 'x' || t.start[1] == 'X'))
                 ? TOK_INT : (memchr(t.start, '.', l->pos - l->pos) ? TOK_FLOAT : TOK_INT);
    ...
    switch (c) {
        ...
        case '$': t.type = TOK_DOLLAR; break;
        case '~': t.type = TOK_TILDE; break;
        case '+':
            if (l->pos < l->len && lexer_peek(l) == '+') { lexer_advance(l); t.type = TOK_PLUS_PLUS; t.length = 2; }
            else if (l->pos < l->len && lexer_peek(l) == '=') { lexer_advance(l); t.type = TOK_PLUS_ASSIGN; t.length = 2; }
            else t.type = TOK_PLUS;
            break;
        case '-':
            if (l->pos < l->len && lexer_peek(l) == '>') { lexer_advance(l); t.type = TOK_ARROW; t.length = 2; }
            else if (l->pos < l->len && lexer_peek(l) == '-') { lexer_advance(l); t.type = TOK_MINUS_MINUS; t.length = 2; }
            else if (l->pos < l->len && lexer_peek(l) == '=') { lexer_advance(l); t.type = TOK_MINUS_ASSIGN; t.length = 2; }
            else t.type = TOK_MINUS;
            break;
        case '*':
            if (l->pos < l->len && lexer_peek(l) == '=') { lexer_advance(l); t.type = TOK_STAR_ASSIGN; t.length = 2; }
            else t.type = TOK_STAR;
            break;
        case '/':
            if (l->pos < l->len && lexer_peek(l) == '=') { lexer_advance(l); t.type = TOK_SLASH_ASSIGN; t.length = 2; }
            else t.type = TOK_SLASH;
            break;
        case '%': t.type = TOK_PERCENT; break;
        case '=':
            if (l->pos < l->len && lexer_peek(l) == '=') { lexer_advance(l); t.type = TOK_EQ; t.length = 2; }
            else if (l->pos < l->len && lexer_peek(l) == '>') { lexer_advance(l); t.type = TOK_FAT_ARROW; t.length = 2; }
            else t.type = TOK_ASSIGN;
            break;
        case '!':
            if (l->pos < l->len && lexer_peek(l) == '=') { lexer_advance(l); t.type = TOK_NE; t.length = 2; }
            else t.type = TOK_BANG;
            break;
        case '<':
            if (l->pos < l->len && lexer_peek(l) == '=') { lexer_advance(l); t.type = TOK_LE; t.length = 2; }
            else t.type = TOK_LT;
            break;
        case '>':
            if (l->pos < l->len && lexer_peek(l) == '=') { lexer_advance(l); t.type = TOK_GE; t.length = 2; }
            else t.type = TOK_GT;
            break;
        case '&':
            if (l->pos < l->len && lexer_peek(l) == '&') { lexer_advance(l); t.type = TOK_AND_AND; t.length = 2; }
            else t.type = TOK_AMP;
            break;
        case '|':
            if (l->pos < l->len && lexer_peek(l) == '|') { lexer_advance(l); t.type = TOK_OR_OR; t.length = 2; }
            else t.type = TOK_PIPE;
            break;
...
static int infix_rbp(TokenType t) {
    switch (t) {
        case TOK_ASSIGN: case TOK_PLUS_ASSIGN:
        case TOK_MINUS_ASSIGN: case TOK_STAR_ASSIGN:
        case TOK_SLASH_ASSIGN:                   return 2;
        default: return 0;
    }
}
...
static int infix_lbp(TokenType t) {
    switch (t) {
        case TOK_OR_OR:                                    return 3;
        case TOK_AND_AND:                                  return 4;
        case TOK_PIPE:                                     return 5;
        case TOK_CARET:                                    return 6;
        case TOK_AMP:                                      return 7;
        case TOK_EQ: case TOK_NE:                          return 8;
        case TOK_LT: case TOK_GT:
        case TOK_LE: case TOK_GE:                          return 9;
...
static int prefix_bp(TokenType t) {
    switch (t) {
        case TOK_MINUS: case TOK_BANG: case TOK_STAR:
...
static int is_expr_start(TokenType t) {
    return t == TOK_IDENT || t == TOK_INT || t == TOK_FLOAT ||
           t == TOK_STRING || t == TOK_CHAR ||
           t == TOK_LPAREN || t == TOK_LBRACKET || t == TOK_LBRACE ||
           t == TOK_MINUS || t == TOK_BANG || t == TOK_STAR ||
           t == TOK_AMP || t == TOK_TILDE ||
           t == TOK_HASH || t == TOK_AT || t == TOK_DOLLAR;
}
...
static AstNodeId pratt_parse_group(PrattParser *p, TokenType close, const char *delim_text) {
...
static AstNodeId pratt_parse_expr(PrattParser *p, int min_bp) {
    ...
    Token tok = p->current;
    switch (tok.type) {
        case TOK_INT: case TOK_FLOAT: case TOK_STRING: case TOK_CHAR:
        case TOK_IDENT: case TOK_HASH: case TOK_AT: case TOK_DOLLAR:
            pratt_advance(p);
    ...
    for (;;) {
        ...
        switch (p->current.type) {
            ...
            {
                Token op = p->current; pratt_advance(p);
            ...
            {
                // func(args)
                Token lp = p->current; pratt_advance(p);
            ...
            {
                // member access
                Token dot = p->current; pratt_advance(p);
            ...
            {
                // 类型标注: expr : type
                Token col = p->current; pratt_advance(p);
