#!/usr/bin/env python3
"""Build the stage-2 compiler used by the normal lainc CLI path."""
...
from build_meta_artifact import build as build_stage1, compiler_artifact_valid
...
def input_fingerprint(source: Path, stage1: Path) -> str:
    return cache_fingerprint(
        ...
        [
            ...
            ("stage1-artifact", stage1.read_bytes()),
...
def build() -> Path:
    ...
    run([
        str(tool("lainc")),
        "--artifact", str(stage1),
        "--emit-l1", str(source), str(OUT),
    ])
    if not compiler_artifact_valid(OUT):
        raise RuntimeError(
            "stage-2 artifact lacks compiler_compile or compiler API schema 1"
        )
