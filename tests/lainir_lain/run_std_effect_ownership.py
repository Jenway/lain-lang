#!/usr/bin/env python3
"""M2.5 acceptance for effect and ownership policy values."""
...
FIXTURE = Path(__file__).parent / "fixtures" / "std_effect_ownership.lain"


def run(*args: Path | str, timeout: float = 300) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        [str(arg) for arg in args], cwd=ROOT, capture_output=True, text=True, timeout=timeout
    )
...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="lainc-std-effect-ownership-") as directory:
        tmp = Path(directory)
        cached = ROOT / "build" / "debug-gen2-heap.l1"
        product = tmp / "std_effect_ownership.l1"
        ...
        checked(
            ...
            ROOT / "std" / "ownership.lain",
            FIXTURE,
        )
        checked(PRINT, product, "main")
        result = checked(SEED, "run", product, "main")
        if result.stdout.strip() != "42":
            raise RuntimeError(f"std effect/ownership result != 42: {result.stdout!r}")
    print("PASS std::effect/ownership policy state machines")
    return 0
