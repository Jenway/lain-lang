#!/usr/bin/env python3
"""M2.3 acceptance for generic parameter and specialization policy values."""
...
FIXTURE = Path(__file__).parent / "fixtures" / "std_generic.lain"
...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="lainc-std-generic-") as directory:
        tmp = Path(directory)
        product = tmp / "std_generic.l1"
        ...
        checked(
            ...
            product,
            ROOT / "std" / "generic.lain",
            FIXTURE,
        )
        checked(PRINT, product, "main")
        result = checked(SEED, "run", product, "main")
        if result.stdout.strip() != "42":
            raise RuntimeError(f"std generic result != 42: {result.stdout!r}")
    print("PASS std::generic parameter/key/recursion policy")
    return 0
