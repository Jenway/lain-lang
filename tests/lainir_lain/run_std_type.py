#!/usr/bin/env python3
"""M2.2 acceptance for the Lain-written standard type policy module."""
...
FIXTURE = Path(__file__).parent / "fixtures" / "std_type.lain"
...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="lainc-std-type-") as directory:
        tmp = Path(directory)
        product = tmp / "std_type.l1"
        ...
        if result.stdout.strip() != "42":
            raise RuntimeError(f"std type result != 42: {result.stdout!r}")
    print("PASS std::type kinds/representation/conversion/overload")
    return 0
