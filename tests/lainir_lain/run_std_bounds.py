#!/usr/bin/env python3
"""M2.4 acceptance for checked/verified/unchecked bounds policy values."""
...
FIXTURE = Path(__file__).parent / "fixtures" / "std_bounds.lain"
...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="lainc-std-bounds-") as directory:
        tmp = Path(directory)
        cached = ROOT / "build" / "debug-gen2-heap.l1"
        product = tmp / "std_bounds.l1"
        ...
        if result.stdout.strip() != "42":
            raise RuntimeError(f"std bounds result != 42: {result.stdout!r}")
    print("PASS std::bounds checked/verified/unchecked policies")
    return 0
