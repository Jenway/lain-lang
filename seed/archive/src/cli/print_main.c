...
int main(int argc, char **argv) {
  ...
  L1Builder *builder = lainir_builder_new(NULL);
  if (!builder) {
    free(source);
    return 2;
  }
  if (!lainir_parse_module_checked(builder, (const char *)source, &module,
                                   &diagnostic)) {
  ...
  if (fold_eval) {
    ...
    if (lainir_fold_module(builder, module, NULL, &error) != LAINIR_RUN_OK) {
