...
/* This unit drives both stages: it folds through eval_fold and runs #eval
 * blocks through the VM control plane.  Step 1/2 of seed/REFACTOR_PLAN.md
 * separates those entries. */
...
/* The handle is the artifact's owner: it holds the builder, and the module is
 * whatever that builder currently has as its root.  Nothing hands out the raw
 * module pointer, so a caller cannot outlive the storage. */
struct LainirModuleHandle { L1Builder *builder; };

static L1Subroutine *handle_module(const LainirModuleHandle *handle) {
  return handle ? lainir_builder_root(handle->builder) : NULL;
}
...
static LainirRunStatus collect_module_eval_values(
  ...
  L1Subroutine *module = handle_module(handle);
  if (values_out) *values_out = NULL;
  if (count_out) *count_out = 0;
  if (error_out) *error_out = NULL;
  if (!module || !values_out || !count_out) {
    if (error_out) *error_out = "invalid module eval arguments";
    return LAINIR_RUN_BAD_CALL;
  }
  if (lainir_fold_module_with_sink(handle->builder, module, caps, error_out,
...
LainirRunStatus lainir_module_parse_handle(
  ...
  if (!builder) return LAINIR_RUN_TRAP;
  if (!lainir_parse_module_checked(builder, source, &module, diagnostic)) {
...
void lainir_module_free_handle(LainirModuleHandle *handle) {
  if (!handle) return;
  lainir_builder_free(handle->builder);
  free(handle);
}
...
const L1Subroutine *lainir_module_handle_find_procedure(
  ...
  for (procedure = handle_module(handle); procedure; procedure = procedure->next) {
...
LainirRunStatus lainir_module_handle_verify_entry(
    ...
    L1Diagnostic *diagnostic) {
  L1Subroutine *module = handle_module(handle);
  if (!module) return LAINIR_RUN_BAD_CALL;
  return lainir_verify_module(module, entry_name, diagnostic)
...
LainirRunStatus lainir_module_handle_fold(
  ...
  L1Subroutine *module = handle_module(handle);
  if (!module) return LAINIR_RUN_BAD_CALL;
  return lainir_fold_module(handle->builder, module, caps, error_out);
}
...
LainirRunStatus lainir_module_handle_eval_block(
  ...
  if (!module || !block || !result_out)
    return LAINIR_RUN_BAD_CALL;
  return lainir_eval_block(module, block, return_type, caps,
...
LainirRunStatus lainir_eval_source(
  ...
  if (lainir_module_handle_fold(handle, caps, error_out) != LAINIR_RUN_OK) {
    lainir_module_handle_destroy(&handle);
    return LAINIR_RUN_BAD_CALL;
  }
  *handle_out = handle;
...
LainirRunStatus lainir_eval_source_text(
  ...
  status = lainir_eval_source(source, caps, &handle, diagnostic, error_out);
  if (status != LAINIR_RUN_OK)
    return LAINIR_RUN_BAD_CALL;
  if (!text_out || !length_out || !lainir_emit_text_module(
          (LainirWriter){.context = &buffer, .write = eval_text_write},
          handle_module(handle), diagnostic)) {
    lainir_module_handle_destroy(&handle);
    free(buffer.data);
