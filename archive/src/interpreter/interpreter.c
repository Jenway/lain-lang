/* This translation unit still implements both sides of the boundary: the VM
 * (execute + control) and the `#eval` fold stage.  Step 2 of
 * seed/REFACTOR_PLAN.md moves fold_* out to src/core/eval_fold.c; until then it
 * needs all four headers. */
...
static L1Expr *fold_value_expr(L1Builder *builder, L1Subroutine *module, LainirValue value,
                               L1Type *return_type, const char **error_out) {
  L1Expr *result = NULL;
  if (value.kind == LAINIR_VALUE_UNIT) return NULL;
  if (value.kind == LAINIR_VALUE_BITS) {
    result = lainir_new_expr(builder, EXPR_CONST);
    if (result) result->data.const_val = (int64_t)value.as.bits;
    return result;
  }
  if (value.kind == LAINIR_VALUE_STRING) {
    result = lainir_new_expr(builder, EXPR_STRING);
    if (result) {
  ...
  if (value.kind == LAINIR_VALUE_FUNC && value.as.func && value.as.func->name) {
    result = lainir_new_expr(builder, EXPR_PROC_ADDR);
  ...
  if (value.kind == LAINIR_VALUE_ADDR) {
    for (L1Subroutine *item = module; item; item = item->next) {
      if (item->is_data && item->data_bytes == value.as.addr) {
        result = lainir_new_expr(builder, EXPR_DATA_ADDR);
        if (result) {
...
static LainirRunStatus fold_expr(L1Builder *builder, L1Subroutine *module, L1Expr **slot,
                                 LainirCapabilityTable *caps,
                                 const char **error_out,
                                 LainirEvalSink sink,
                                 void *sink_user_data);
...
static LainirRunStatus fold_block(L1Builder *builder, L1Subroutine *module, L1Block *block,
                                  LainirCapabilityTable *caps,
                                  const char **error_out,
                                  LainirEvalSink sink,
                                  void *sink_user_data) {
  for (; block; block = block->next) {
    for (L1Instruction *inst = block->body; inst; inst = inst->next) {
      ...
      case INST_STORE:
        if (fold_expr(builder, module, &inst->data.store.dest, caps, error_out, sink, sink_user_data) != LAINIR_RUN_OK) return LAINIR_RUN_TRAP;
      ...
      case INST_IF:
        ...
        if (fold_expr(builder, module, expr, caps, error_out, sink, sink_user_data) != LAINIR_RUN_OK) return LAINIR_RUN_TRAP;
        if (fold_block(builder, module, inst->data.if_stmt.then_body, caps, error_out, sink, sink_user_data) != LAINIR_RUN_OK) return LAINIR_RUN_TRAP;
        if (fold_block(builder, module, inst->data.if_stmt.else_body, caps, error_out, sink, sink_user_data) != LAINIR_RUN_OK) return LAINIR_RUN_TRAP;
        continue;
      case INST_LOOP:
        if (fold_block(builder, module, inst->data.loop.body, caps, error_out, sink, sink_user_data) != LAINIR_RUN_OK) return LAINIR_RUN_TRAP;
        continue;
      default: continue;
      }
      if (expr && fold_expr(builder, module, expr, caps, error_out, sink, sink_user_data) != LAINIR_RUN_OK) return LAINIR_RUN_TRAP;
    }
  }
  return LAINIR_RUN_OK;
}
...
static LainirRunStatus fold_expr(L1Builder *builder, L1Subroutine *module, L1Expr **slot,
  ...
  case EXPR_EVAL: {
    ...
    fold_eval_depth++;
    nested_status = fold_block(builder, module, expr->data.eval.block, caps, error_out,
                               sink, sink_user_data);
    fold_eval_depth--;
    ...
    L1Expr *constant = fold_value_expr(builder, module, value, expr->data.eval.ret_ty,
                                       error_out);
    ...
    *slot = constant;
    /* The replaced node stays allocated: the builder that owns this artifact
     * releases it together with everything else. */
    return LAINIR_RUN_OK;
  }
  case EXPR_LOAD: return fold_expr(builder, module, &expr->data.load.addr, caps, error_out, sink, sink_user_data);
  case EXPR_LEA:
    if (fold_expr(builder, module, &expr->data.lea.base, caps, error_out, sink, sink_user_data) != LAINIR_RUN_OK) return LAINIR_RUN_TRAP;
    return fold_expr(builder, module, &expr->data.lea.idx, caps, error_out, sink, sink_user_data);
  case EXPR_CALL: for (uint32_t i = 0; i < expr->data.call.arg_count; i++) if (fold_expr(builder, module, &expr->data.call.args[i], caps, error_out, sink, sink_user_data) != LAINIR_RUN_OK) return LAINIR_RUN_TRAP; return LAINIR_RUN_OK;
  case EXPR_CALL_INDIRECT:
    if (fold_expr(builder, module, &expr->data.call_indirect.fn_ptr, caps, error_out, sink, sink_user_data) != LAINIR_RUN_OK) return LAINIR_RUN_TRAP;
    for (uint32_t i = 0; i < expr->data.call_indirect.arg_count; i++) if (fold_expr(builder, module, &expr->data.call_indirect.args[i], caps, error_out, sink, sink_user_data) != LAINIR_RUN_OK) return LAINIR_RUN_TRAP;
    return LAINIR_RUN_OK;
  ...
  case EXPR_FADD: case EXPR_FSUB: case EXPR_FMUL: case EXPR_FDIV: case EXPR_FEQ: case EXPR_FLT:
    if (fold_expr(builder, module, &expr->data.bin.left, caps, error_out, sink, sink_user_data) != LAINIR_RUN_OK) return LAINIR_RUN_TRAP;
    return fold_expr(builder, module, &expr->data.bin.right, caps, error_out, sink, sink_user_data);
  case EXPR_POPCOUNT: case EXPR_CLZ: case EXPR_ROTL: case EXPR_INT2PTR: case EXPR_PTR2INT:
    return fold_expr(builder, module, &expr->data.unary.operand, caps, error_out, sink, sink_user_data);
  case EXPR_ZEXT: case EXPR_SEXT: case EXPR_TRUNC: case EXPR_BITCAST:
    return fold_expr(builder, module, &expr->data.conversion.operand, caps, error_out, sink, sink_user_data);
...
LainirRunStatus lainir_fold_module_with_sink(L1Builder *builder,
                                             L1Subroutine *module,
                                             LainirCapabilityTable *caps,
                                             const char **error_out,
                                             LainirEvalSink sink,
  ...
  fold_eval_depth = 0;
  for (L1Subroutine *sub = module; sub; sub = sub->next)
    if (fold_block(builder, module, sub->blocks, caps, error_out, sink, sink_user_data) != LAINIR_RUN_OK)
      return LAINIR_RUN_TRAP;
  if (error_out) *error_out = NULL;
  return LAINIR_RUN_OK;
}

LainirRunStatus lainir_fold_module(L1Builder *builder,
                                   L1Subroutine *module,
                                   LainirCapabilityTable *caps,
                                   const char **error_out) {
  return lainir_fold_module_with_sink(builder, module, caps, error_out, NULL, NULL);
}
