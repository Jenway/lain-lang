  ...
  (let* ((error-expr (perform.extract-error-expr expr)))
    (if (not error-expr)
        ...
        (let* ((flag-ty (ir.type.bits 8))
               (error-code (core.lower-expr block error-expr (ir.type.bits 32) locals))
               (flag-one (ir.expr.const block flag-ty 1))
               (value-zero (ir.expr.const block (ir.type.bits 32) 0))
               (field-types (list flag-ty (ir.type.bits 32) (ir.type.bits 32)))
